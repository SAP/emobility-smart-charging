...
- `-P` Publish all container's ports to the host: Change the port in `EXPOSE 8080` to adjust which port you want the application to run on
```
docker run -d -P emobility_smart_charging
```
