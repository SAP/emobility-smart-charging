...
public class InstanceHeuristicRelSoCLP extends InstanceHeuristicLP {

	/**
	 * First assigns cars based on their relative state of charge (SoC), then 
	 ...
	 * @param fuseTree
	 */
	public InstanceHeuristicRelSoCLP(List<Car> cars, List<ChargingStation> chargingStations,
	...
	public InstanceHeuristicRelSoCLP(DataGenerator data) {
		this(data.getCars(),
	...
	public void prepareInstanceLP() {
		try (MethodTimer t = new MethodTimer(this.timeProblemConstruction)) {
			...
			List<Integer> sortedCarIDs = CarAssignmentPriority.sortCarIdByRelSoC(this.getCars());
