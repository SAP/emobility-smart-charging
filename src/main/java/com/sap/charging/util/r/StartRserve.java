...
public class StartRserve {
	...
	public static boolean launchRserve(String cmd) {
		return launchRserve(cmd, 6311);
	}
	
	/** shortcut to <code>launchRserve(cmd, "--no-save --slave", "--no-save --slave", false)</code> */
	public static boolean launchRserve(String cmd, int port) { 
	 ...
	 */
	public static boolean launchRserve(String cmd, String rargs, String rsrvargs, boolean debug, int port) {
		try {
			Process p;
			...
			if (osname != null && osname.length() >= 7 && osname.substring(0,7).equals("Windows")) {
				...
				String[] commands = new String[] {
					      "/bin/sh", "-c",
					      	"echo 'library(Rserve);" + 
					      	"Rserve(" + (debug?"TRUE":"FALSE") + ", port=" + port + ", args=\""+rsrvargs+"\")'" + 
					      	"|"+cmd+" "+rargs
					      };
	...
	public static boolean checkLocalRserve(int port) {
		//System.out.println("Checking for local Rserve on port=" + port);
		...
		return (launchRserve("R", port) || /* try some common unix locations of R */
			((new File("/Library/Frameworks/R.framework/Resources/bin/R")).exists() && launchRserve("/Library/Frameworks/R.framework/Resources/bin/R", port)) ||
	...
	public static void main(String[] args) {
		System.out.println("result="+checkLocalRserve(6311));
		try {
			RConnection c=new RConnection();
			c.shutdown();
		} catch (Exception x) {};
