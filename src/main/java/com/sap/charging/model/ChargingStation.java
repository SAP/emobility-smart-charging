...
/**
 * Also: Loading Spot, Charging Spot, ...
 * Currently, all fuses directly at a charging station can hold 32A
 */
...
public class ChargingStation implements FuseTreeNode {
    ...
    public enum StationStatus {
            ...
            Reserved /*The charge station is currently reserved for a cetain car and not available to everyone.*/,
            Blocked /*The charge station is blocked, either because a charging session is over or because a different car parked there.*/,
            Maintenance /*Charge station is in maintenance mode. No charge sessions possible.*/,
            Disconnected /*The connection to the charge station was lost and therefore no information is available.*/
    };
     ...
     * Example:
     *     chargingStation i0: 1->1; 2->2; 3->3
     *     chargingStation i1: 1->2; 2->3; 3->1
     *     chargingStation i2: 1->3; 2->1; 3->2
     *     chargingStation i3: 1->1; 2->2; 3->3
     *     ...
     *     ("Phasenwanderung")
     */
    ...
    public ChargingStation(
    		...
    		@JsonProperty("fusePhase1") double fusePhase1, 
    		@JsonProperty("fusePhase2") double fusePhase2, 
    	...
    	this.sanityCheckPhases(phaseToGrid, phaseToChargingStation); 
    ...
    private void sanityCheckPhases(HashMap<Phase, Phase> phaseToGrid, HashMap<Phase,Phase> phaseToChargingStation) {
    	if ((phaseToGrid == null && phaseToChargingStation != null) || 
    		(phaseToGrid != null && phaseToChargingStation == null)) {
    		throw new RuntimeException("chargingStation.phaseToGrid and chargingStation.phaseToChargingStation must either both be null or both set as parameters."); 
    	}
    }
    ...
    public String toString() {
    	...
    	if (phaseToGrid != null) {
    		result += "Matching: " + "1->"
                    + phaseToGrid.get(Phase.PHASE_1).asInt() + "; " + "2->" + phaseToGrid.get(Phase.PHASE_2).asInt() + "; "
                    + "3->" + phaseToGrid.get(Phase.PHASE_3).asInt() + ", "; 
    	}
    	result += "isBEVAllowed=" + isBEVAllowed + ", "
                + "isPHEVAllowed=" + isPHEVAllowed;
    	return result; 
    }
